...
// 8K:
#define FRAMEBUFFER_WIDTH  7680
#define FRAMEBUFFER_HEIGHT 4320
...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        tex_color = loadTexture2D("../Modele/" TEXTURE);
...
{
    qDebug() << "Zapisuje color buffer...";
    QImage tex(FRAMEBUFFER_WIDTH, FRAMEBUFFER_HEIGHT, QImage::Format_RGBA8888);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_color);
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, tex.bits());
    tex.mirrored(false, true).save("C:\\temp\\colorbuffer.jpg");

    qDebug() << "Zapisuje depth buffer...";
    QImage tex_d(FRAMEBUFFER_WIDTH, FRAMEBUFFER_HEIGHT, QImage::Format_Grayscale8);
    glBindTexture(GL_TEXTURE_2D, tex_FBO_depth);
    glGetTexImage(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, tex_d.bits());
    tex_d.mirrored(false, true).save("C:\\temp\\depthbuffer.jpg");
...
void WidgetOpenGL::resizeGL(int w, int h)
{
    ...
    r = float(FRAMEBUFFER_WIDTH)/float(FRAMEBUFFER_HEIGHT);
