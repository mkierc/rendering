...
#define MODEL         "dragon" // dragon
#define MODEL_TEXTURE "dragonskingold" // dragonskingold
...
void WidgetOpenGL::paintAndShade()
{
    ...
    int attr_w = getUniformLocation(shaderProgram_shading, "width");
    glUniform1i(attr_w, width());

    int attr_h = getUniformLocation(shaderProgram_shading, "height");
    glUniform1i(attr_h, height());

    int attr_e = getUniformLocation(shaderProgram_shading, "edge");
    glUniform1f(attr_e, edge);
...
void WidgetOpenGL::set_edge(float _edge)
{
    edge = _edge;
