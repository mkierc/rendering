...
void WidgetOpenGL::initializeGL()
{
    ...
    try
    {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
            {
                bezier[3*(i + 4*j) + 0] = i/3.0 - 0.5;
                bezier[3*(i + 4*j) + 1] = j/3.0 - 0.5;
                bezier[3*(i + 4*j) + 2] = (qrand()%100 - 50)/75.0;
            }
        ...
        glBufferData(GL_ARRAY_BUFFER, 48*sizeof(GLfloat), bezier, GL_STATIC_DRAW);
...
//        glCullFace(GL_BACK);
        ...
        tessInner0 = tessInner1 = tessOuter0 = tessOuter1 = tessOuter2 = tessOuter3 = 14.0;
