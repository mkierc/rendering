...
uniform float texNoiseDens;
uniform float texNoiseFactor;
uniform int   texNoiseSpect;
uniform float texPow;
...
vec3 texPerlin(vec3 p)
{
  const vec3 color1 = vec3(0.0, 0.7, 0.0);
  const vec3 color2 = vec3(0.2, 0.2, 0.2);

  float f = 0;
  float w = 1;
  for (int i = 0; i < texNoiseSpect; i++)
  {
    f += cnoise(p*texNoiseDens*w)/w;
    w *= 2;
  }
  return mix(color1, color2, (f + 2.0)*0.25);
}
...
//  const vec3 color2 = vec3(188, 114, 77)/255.0;
  ...
  float f = texSize*sqrt(p.x*p.x + p.y*p.y);
  float w = 1;
  for (int i = 0; i < texNoiseSpect; i++)
  {
    f += cnoise(p.xy*texNoiseDens*w)*texNoiseFactor/w;
    w *= 2;
  }

  float s = fract(f); // = f - int(f);
  s = pow(abs(s), texPow);
...
vec3 texMarble(vec3 p)
{
  ...
  const vec3 color2 = vec3(1.0, 0.3, 0.0);
  const vec3 color3 = vec3(0.0, 0.7, 0.9);

//  const vec3 color1 = vec3(0.0, 0.3, 0.9);
//  const vec3 color2 = vec3(1.0, 1.0, 1.0);
//  const vec3 color3 = vec3(0.0, 1.0, 1.0);

//  const vec3 color1 = vec3(1);
//  const vec3 color2 = vec3(0);
//  const vec3 color3 = vec3(1, 0, 0);
  ...
  for (int i = 0; i < texNoiseSpect; i++)
  {
    f += cnoise(p.xyz*texNoiseDens*w)*texNoiseFactor/w;
    w *= 2;
  }
  ...
  s = pow(abs(s), texPow);
  if (s < 0.5)
    return mix(color1, color2, s*2);
  else
    return mix(color2, color3, (s-0.5)*2);
}
